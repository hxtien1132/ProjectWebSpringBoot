...
public class RegisterController {
	...
	public String register(ModelMap model, @Validated @ModelAttribute("user") User dto, BindingResult result,
		...
		if (result.hasErrors()) {// check errors lỗi trả về form đăng ký
			return "web/register";
		}
		if (!checkEmail(dto.getEmail())) {// check email đã tồn tại hay chưa
			model.addAttribute("error", "Email này đã được sử dụng!");
			return "web/register";
		}
		session.removeAttribute("otp");
		int random_otp = (int) Math.floor(Math.random() * (999999 - 100000 + 1) + 100000);// random 6 chữ số otp
		session.setAttribute("otp", random_otp);
		String body = "<div>\r\n" + "<h3>Mã xác thực OTP của bạn là: <span style=\"color:#119744; font-weight: bold;\">"
				+ random_otp + "</span></h3>\r\n" + "</div>";
		sendMailService.queue(dto.getEmail(), "Đăng kí tài khoản", body);// gửi otp qua email
